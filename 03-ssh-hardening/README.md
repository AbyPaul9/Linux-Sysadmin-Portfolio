...
- Changed default SSH from port 22 to port 2222
...
- Temporarily re-enabled to allow ssh-sopy-id, then disabled again after keys were copied

### 3. Change default SSH port
- set port to 2222 in /etc/ssh/sshd_config
- semanage port -a -t ssh_port_t -p tcp 2222
- firewall-cmd --add-port=2222/tcp --permanent
- firewall-cmd --reload to open firewalld
...
### 5. Set Idle Timeout
- File Edited: /etc/ssh/ssh_config
...
- ssh -p ops.mike@192.168.56.10 to test login
...
- Key-Based suthentication is significantly more secured than passwords. Private keys cannot be brute-forced remotely.
...
## Lesson Learnt
- Disabling PasswordAuthentication before copying SSH keys create a lockout situation(Always copy keys first then disable afterwards)
